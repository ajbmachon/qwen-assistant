...
from . import api_keys, auth, data_protection
from . import logging as sec_logging
...
def get_api_key_manager() -> api_keys.ApiKeyManager:
    return api_keys.get_api_key_manager()
...
def get_data_protection() -> data_protection.DataProtection:
    return data_protection.get_data_protection()


def get_request_validator() -> validation.RequestValidator:
    return validation.get_request_validator()


def get_response_validator() -> validation.ResponseValidator:
    return validation.get_response_validator()


def get_security_logger(log_dir: Optional[str] = None) -> sec_logging.SecurityLogger:
    return sec_logging.get_security_logger(log_dir)
...
class SecurityManager:
    ...
    def __init__(
        self, config: Optional[Dict[str, Any]] = None, log_dir: Optional[str] = None
    ):
        """
    ...
    def end_session(self, session_id: str) -> bool:
        ...
        # Log session end
        self.security_logger.log_auth_event(
            message="Session ended", success=result, details={"session_id": session_id}
        )
    ...
    def validate_tool_call(
        self, tool_name: str, parameters: Dict[str, Any], user_id: Optional[str] = None
        ...
        Returns:
            Tuple of (is_valid, error_message or None)
        """
        is_valid, error = self.request_validator.validate_tool_parameters(
    ...
    def sanitize_response(
        ...
        if not is_valid:
            ...
            return self.response_validator.prepare_safe_response(
                success=False, error="Internal response format error"
            )
...
def get_security_manager(
    config: Optional[Dict[str, Any]] = None, log_dir: Optional[str] = None
) -> SecurityManager:
    """
