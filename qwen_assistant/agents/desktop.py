...
class DesktopAgent(BaseAgent):
    ...
    def can_handle(self, request: Dict[str, Any]) -> float:
        ...
        desktop_keywords = {
            ...
            "high": ["run command", "execute command", "terminal command", "shell command", 
                     "system command", "search files", "list directory", "write file", 
                     "read file", "create file", "edit file", "delete file", "rename file"],
                     
            # Medium confidence keywords (0.2)
            "medium": ["file", "directory", "folder", "command", "terminal", "shell", 
                      "run", "execute", "script", "find", "code", "local", "computer"]
        }
    ...
    async def _mcp_call(self, tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.mcp_endpoint}/api/v1/execute", 
                json=payload
            ) as response:
    ...
    async def search_files(self, directory: str, pattern: str) -> Dict[str, Any]:
        ...
        """
        return await self._mcp_call("search_files", {
            "directory": directory,
            "pattern": pattern
        })
    ...
    async def search_code(self, directory: str, query: str) -> Dict[str, Any]:
        ...
        Returns:
            Search results
        """
        return await self._mcp_call("search_code", {
            "directory": directory,
            "query": query
        })
    ...
    async def handle_request(self, request: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            elif action == "search_files" or "find files" in query or "search files" in query:
            ...
            elif action == "search_code" or "find code" in query or "search code" in query:
                directory = request.get("directory", ".")
